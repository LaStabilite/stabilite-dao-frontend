...
export const zapinLpData = async (
  ...
  const { _swapTarget, swapData } = zapinInterface.decodeFunctionData(
    "ZapIn",
    data.data,
  );
...
export const zapinData = async (
  ...
  const buyAmount = dataBuyAmount.sub(
    dataBuyAmount.mul(slippage * 1000).div(1000),
  );
